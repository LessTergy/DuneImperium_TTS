-- Bundled by luabundle {"rootModuleName":"Board Main.2da390.lua","version":"1.6.0"}
...
__bundle_register("Board Main.2da390.lua", function (require, _LOADED, __bundle_register, __bundle_modules)
    i18n = require("i18n")
    require("locales")
    ...
    boardReputationModule = require("BoardReputationModule")
    ...
    function HarvestSpice(color, base, bonusRef)
        ...
        bonusRef.call("reset_val")
        for i = 1, nbSpice, 1 do
            Wait.time(function () ressources[color][1].call("incrementVal") end, t)
            t = t + (1.5 / nbSpice)
        end
        return nbSpice
    end

    local bonus_spice1 = getObjectFromGUID('3cdb2d')
    local bonus_spice2 = getObjectFromGUID('394db2')
    local bonus_spice3 = getObjectFromGUID('116807')

    function processHarvestSpace(color, baseSpice, bonusSpice, messageIndex)
        local leader = ressources[color][6].getObjects()[1]
        local leaderName = leader.getName()
        if leader.hasTag("Ariana") then
            local nbSpice = HarvestSpice(color, baseSpice - 1, bonusSpice)
            ...
            if not enoughCards then
                broadcastToAll(
                    i18n(messageIndex):format(leaderName, nbSpice) .. ".", color)
                ...
                Player[color].showConfirmDialog(
                    ...
                    function (player_color)
                        DrawOne(_, color)
                        broadcastToAll(i18n(messageIndex .. "Draw"):format(
                            leaderName), color)
                    end)
            else
                DrawOne(_, color)
                broadcastToAll(i18n(messageIndex):format(leaderName, nbSpice) ..
                    i18n("et") .. i18n("drawOneCard"), color)
            end
        else
            local nbSpice = HarvestSpice(color, baseSpice, bonusSpice)
            broadcastToAll(i18n(messageIndex):format(leaderName, nbSpice) .. ".",
                color)
        end
    end
    ...
    function ImperialBasin(_, color)
        ...
        local ImperialBasin_zone = getObjectFromGUID("2c77c1")

        if boardCommonModule.CheckAgentAndPlayer(color, ImperialBasin_zone) then
            processHarvestSpace(color, 1, bonus_spice1, "imperialBasin")
    ...
    function TheGreatFlat(_, color)
        ...
        local TheGreatFlat_zone = getObjectFromGUID("69f925")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, TheGreatFlat_zone) then
            ...
            if waterObj.call("collectVal") < 2 then
                ...
                waterObj.call("decrementVal")
                waterObj.call("decrementVal")
                processHarvestSpace(color, 3, bonus_spice3, "greatFlat")
            end
        end
    end
    ...
    function Stillsuits(_, color)
        ...
        local Stillsuits_zone = getObjectFromGUID("556f43")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Stillsuits_zone) then
            ...
            ressources[color][3].call("incrementVal")
            broadcastToAll(i18n("stillSuits"):format(leaderName), color)
            if leaderName == "Shaddam IV" then
                boardReputationModule.FremenDown(color)
            else
                boardReputationModule.FremenUp(color)
            end
        end
    end
    ...
    function HardyWarriors(_, color)
        ...
        local HardyWarriors_zone = getObjectFromGUID("a2fd8e")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, HardyWarriors_zone) then
            ...
            if waterObj.call("collectVal") < 1 then
                ...
                waterObj.call("decrementVal")
                Wait.time(function ()
                    ressources[color][4].call("AddTroop2")
                end, 0.25, 2)
    ...
    function Secrets(_, color)
        ...
        local Secrets_zone = getObjectFromGUID("1f7c08")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Secrets_zone) then
            ...
            if leaderName == "Shaddam IV" then
                boardReputationModule.BeneDown(color)
            else
                boardReputationModule.BeneUp(color)
            end
        end
    end
    ...
    function Foldspace(_, color)
        ...
        local Zone_Cards_foldspace = getObjectFromGUID("6b62e0")
        local Foldspace_zone = getObjectFromGUID("9a9eb5")

        if boardCommonModule.CheckAgentAndPlayer(color, Foldspace_zone) then
            local deck_foldspace = Zone_Cards_foldspace.getObjects()
            local leaderName = ressources[color][6].getObjects()[1].getName()

            if deck_foldspace[1].type == "Deck" then
                deck_foldspace[1].takeObject({
                    position = pos_discard[color],
                    rotation = { 0, 180, 0 },
                    smooth = false
                })
            elseif deck_foldspace[1].type == "Card" then
                deck_foldspace[1].setPosition(pos_discard[color])
            end
            broadcastToAll(i18n("foldspace"):format(leaderName), color)
            if leaderName == "Shaddam IV" then
                boardReputationModule.GuildDown(color)
            else
                boardReputationModule.GuildUp(color)
            end
        end
    end
    ...
    function Heighliner(_, color)
        ...
        local Heighliner_zone = getObjectFromGUID("8b0515")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Heighliner_zone) then
            ...
            if spiceObj.call("collectVal") < 6 then
                ...
                Wait.time(function () spiceObj.call("decrementVal") end, 0.2, 6)
    ...
    function Wealth(_, color)
        ...
        local Wealth_zone = getObjectFromGUID("b2c461")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Wealth_zone) then
            ...
            Wait.time(function () ressources[color][2].call("incrementVal") end, 0.3,
                solari)

            broadcastToAll(i18n("wealth"):format(leaderName, solari), color)

            if leaderName == "Shaddam IV" then
                boardReputationModule.EmperorDown(color)
            else
                boardReputationModule.EmperorUp(color)
            end
        end
    end
    ...
    function Conspire(_, color)
        ...
        local Conspire_zone = getObjectFromGUID("cd9386")
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Conspire_zone) then
            ...
            if spiceObj.call("collectVal") < 4 then
                ...
                Wait.time(function () spiceObj.call("decrementVal") end, 0.3, 4)
    ...
    function SietchTabr(_, color)
        ...
        local SietchTabr_zone = getObjectFromGUID("5bc970")

        if boardCommonModule.CheckAgentAndPlayer(color, SietchTabr_zone) then
            if pion_reput["fremen"][color].getPosition().z < -9.25 then
    ...
    function ResearchStation(_, color)
        self.clearButtons()
        Wait.time(activateButtons, 1)
        if boardCommonModule.CheckAgentAndPlayer(color,
                boardCommonModule.ResearchStation_zone) then
            local waterObj = ressources[color][3]
            local leaderName = ressources[color][6].getObjects()[1].getName()
    ...
    function Carthag(_, color)
        ...
        local Carthag_zone = getObjectFromGUID("b1c938")
    ...
    function Arrakeen(_, color)
        ...
        local Arrakeen_zone = getObjectFromGUID("17b646")
    ...
    function BonusSpatial(color)
        ...
        if leader.hasTag("Yuna") and combat == 0 then solariAmount = 4 end

        for i = 1, solariAmount, 1 do
            t0 = t0 + 0.2
            Wait.time(function () ressources[color][2].call("incrementVal") end, t0)
        end
        broadcastToAll(i18n("spaceBonus"):format(leaderName, solariAmount), color)
    end

    function BonusEmperor(color)
        local leaderName = ressources[color][6].getObjects()[1].getName()

        ressources[color][4].call("AddTroop2")
        Wait.time(function () ressources[color][4].call("AddTroop2") end, 0.25)
    ...
    function onObjectEnterScriptingZone(zone, enter_object)
        if zone.guid == zone_alliance["Emperor"].guid then
            if enter_object.hasTag("EmperorInfluence") then
                for _, color in ipairs(colors) do
                    if enter_object.hasTag(color) then
                        BonusEmperor(color)
                    end
                end
            end
        ...
        if zone.guid == zone_alliance["Spacing Guild"].guid then
            if enter_object.hasTag("SpaceGuildInfluence") then
                for _, color in ipairs(colors) do
                    if enter_object.hasTag(color) then
                        BonusSpatial(color)
                    end
        ...
        if zone.guid == zone_alliance["Bene Gesserit"].guid then
            if enter_object.hasTag("BeneGesseritInfluence") then
                for _, color in ipairs(colors) do
                    if enter_object.hasTag(color) then
                        BonusBene(color)
                    end
        ...
        if zone.guid == zone_alliance["Fremen"].guid then
            if enter_object.hasTag("FremenInfluence") then
                for _, color in ipairs(colors) do
                    if enter_object.hasTag(color) then
                        BonusFremen(color)
                    end
    ...
    local flag_basin_zone = getObjectFromGUID("3fe117")
    local flag_arrakeen_zone = getObjectFromGUID("f1f53d")
    local flag_carthag_zone = getObjectFromGUID("9fc2e1")

    function getColorControllingSpace(zone)
        local dread = false
        local objectsInZone = zone.getObjects()
        local colorGettingReward = nil

        for _, obj in ipairs(objectsInZone) do
            if obj.hasTag("Dreadnought") then
                for _, color in ipairs(colors) do
                    if obj.hasTag(color) then
                        dread = true
                        colorGettingReward = color
                    end
                end
            end

            if obj.hasTag("Flag") and dread == false then
                for _, color in ipairs(colors) do
                    if obj.hasTag(color) then
                        colorGettingReward = color
                    end
        ...
        return colorGettingReward
    end

    function flag_basin()
        local colorGettingReward = getColorControllingSpace(flag_basin_zone)

        if colorGettingReward ~= nil then
            local leader = ressources[colorGettingReward][6].getObjects()[1]
            local leaderName = leader.getName()
            ressources[colorGettingReward][1].call("incrementVal")
            broadcastToAll(leaderName .. i18n("flagBasin"), { 0.956, 0.392, 0.113 })
        end
    end

    function rewardSolariForControl(playerColor, colorGettingReward, messageIndex)
        if colorGettingReward ~= nil then
            local leader = ressources[colorGettingReward][6].getObjects()[1]
            local leaderName = leader.getName()
            local income = 1

            if leaderName == "Princess Yuna" and colorGettingReward == playerColor then
                income = 2
            end

            Wait.time(function ()
                ressources[colorGettingReward][2].call("incrementVal")
            end, 0.3, income)
            broadcastToAll(leaderName .. i18n(messageIndex):format(income),
                { 0.8, 0.8, 0.8 })
        end
    end

    function flag_arrakeen(playerColor)
        local colorGettingReward = getColorControllingSpace(flag_arrakeen_zone)

        rewardSolariForControl(playerColor, colorGettingReward, "flagArrakeen")
    end

    function flag_carthag(playerColor)
        local colorGettingReward = getColorControllingSpace(flag_carthag_zone)

        rewardSolariForControl(playerColor, colorGettingReward, "flagCarthag")
    end
end)
__bundle_register("BoardReputationModule", function (require, _LOADED, __bundle_register, __bundle_modules)
...
return __bundle_require("Board Main.2da390.lua")
