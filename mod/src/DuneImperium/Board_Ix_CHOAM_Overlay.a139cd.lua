-- Bundled by luabundle {"rootModuleName":"Board Ix CHOAM Overlay.a139cd.lua","version":"1.6.0"}
...
__bundle_register("Board Ix CHOAM Overlay.a139cd.lua", function (require, _LOADED, __bundle_register, __bundle_modules)
    i18n = require("i18n")
    require("locales")
    ...
    function gainSolariFromSmuggling(color)
        ...
        Wait.time(function () ressources[color][2].call("incrementVal") end, 0.25,
            solariIncome)
    ...
    function cargoUp(color)
        ...
        if cargo[color].getPosition().z > 9.6 then
            broadcastToColor(i18n("nope"), color, "Purple")
        else
            cargo[color].setPositionSmooth(cargo[color].getPosition() +
                Vector({ 0, 0, 1.1 }), false, true)
            cargoMoved = true
        end
    ...
    function cargoReset(color)
        ...
        if cargo[color].getPosition().z < 7.5 then
            broadcastToColor(i18n("nope"), color, "Purple")
        else
            cargo[color].setPositionSmooth(cargo_pos_init[color], false, true)
            cargoMoved = true
        end
    ...
    function ShippingUpUp(_, color)
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Shipping_zone) then
            if pion_reput_guild[color].getPosition().z < minimum_reput_guild_z then
                broadcastToColor(i18n("notReputation"), color, "Red")
            elseif cargo[color].getPosition().z > 8.5 then
                broadcastToColor(i18n("nope"), color, "Purple")
            else
                cargo[color].setPositionSmooth(
                    cargo[color].getPosition() + Vector({ 0, 0, 1.1 }), false, true)
                Wait.time(function ()
                    cargo[color].setPositionSmooth(
                        cargo[color].getPosition() + Vector({ 0, 0, 1.1 }), false,
                        true)
                end, 0.5)
                local leaderName = ressources[color][6].getObjects()[1].getName()
    ...
    function ShippingUpDown(_, color)
        ...
        if boardCommonModule.CheckAgentAndPlayer(color, Shipping_zone) then
            ...
            elseif cargoUp(color) then
                local leaderName = ressources[color][6].getObjects()[1].getName()
                Wait.time(function ()
                    cargo[color].setPositionSmooth(cargo_pos_init[color], false,
                        true)
                end, 0.5)
    ...
    function CommercialMoney(_, color)
        ...
        for i = 1, solariIncome - 1, 1 do
            t = t + 0.25
            Wait.time(function () ressources[color][2].call("incrementVal") end, t)
        end
        ressources["Red"][2].call("incrementVal")
        ressources["Blue"][2].call("incrementVal")
        ressources["Green"][2].call("incrementVal")
        ressources["Yellow"][2].call("incrementVal")
    ...
    function CommercialTroop(_, color)
        ...
        local techs = zone_player[color].getObjects()
        ...
        for i = 1, troopsToGain, 1 do
            t = t + 0.25
            Wait.time(function () ressources[color][4].call("AddTroop2") end, t)
        end
    end
end)
...
return __bundle_require("Board Ix CHOAM Overlay.a139cd.lua")
