-- Bundled by luabundle {"rootModuleName":"First Conflict.cb0478.lua","version":"1.6.0"}
...
__bundle_register("First Conflict.cb0478.lua", function (require, _LOADED, __bundle_register, __bundle_modules)
    function onLoad()
    ...
    function RevealConflict()
        local setupObject = getObjectFromGUID("4a3e76")
        local players = setupObject.call("getPlayersBasedOnHotseat")
        ...
        local clock_mode = setupObject.getVar("clock_mode")

        if clock_mode == 1 then
            local clockBag = getObjectFromGUID("302c1d")
            local clock = {
                ["Red"] = { pos = { -31.90, 1.89, 23.42 } },
                ["Green"] = { pos = { 31.95, 1.88, 23.41 } },
                ["Blue"] = { pos = { -31.90, 1.89, 1.26 } },
                ["Yellow"] = { pos = { 31.95, 1.89, 1.23 } }
            }

            for color, params in pairs(clock) do
                local params1 = {}
                params1.position = clock[color].pos
                params1.rotation = { 90, 0, 0 }
                params1.callback_function = function (obj)
                    obj.lock()
                    if Turns.turn_color == color then
                        obj.Clock.paused = false
                    end
                end
                clockBag.takeObject(params1)
            end
        end
...
return __bundle_require("First Conflict.cb0478.lua")
