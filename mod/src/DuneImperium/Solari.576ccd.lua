-- Bundled by luabundle {"rootModuleName":"Solari.576ccd.lua","version":"1.6.0"}
...
__bundle_register("Solari.576ccd.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
MIN_VALUE = 0
MAX_VALUE = 99
...
function add_subtract(_obj, _color, alt_click)
    ...
    if _color ~= "Red" and numberOfPlayers < 3 then
        playerActingStr = i18n("playerActing"):format(i18n(_color:lower()))
        msgColor = "Pink"
    end

    if _color ~= "Red" and numberOfPlayers > 2 then
        ...
        if val ~= new_value then
            ...
            scheduledInQueue = Wait.time(function()
                ...
                if delta < 0 then
                    broadcastToAll(i18n("spentManually"):format(i18n("red"), math.abs(delta),
                                                                solari) ..
                                       playerActingStr, msgColor)

                end
                if delta > 0 then
                    broadcastToAll(i18n("receiveManually"):format(i18n("red"), math.abs(delta),
                                                                solari) ..
                                       playerActingStr, msgColor)
...
return __bundle_require("Solari.576ccd.lua")
