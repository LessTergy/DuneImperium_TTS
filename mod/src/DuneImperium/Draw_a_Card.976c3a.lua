-- Bundled by luabundle {"rootModuleName":"Draw a Card.976c3a.lua","version":"1.6.0"}
...
__bundle_register("Draw a Card.976c3a.lua", function (require, _LOADED, __bundle_register, __bundle_modules)
    ...
    function drawCard(_, color)
        local targetDeck = GetDeckOrCard(intrigue_zone)
        if targetDeck == nil then
            ...
            if targetDeck.type == "Deck" then
                ...
                targetDeck.setPosition({ handZone.position.x - 7.5, handZone.position.y, handZone.position.z }, false,
                    true)
                targetDeck.setRotation({ deckRot.x, deckRot.y, deckRot.z + 180 })
            end
            local leaderName = constants.ressources[color][6].getObjects()[1].getName()
            broadcastToAll(leaderName .. i18n("drawIntrigue"), color)
        end
    end
end)
...
return __bundle_require("Draw a Card.976c3a.lua")
