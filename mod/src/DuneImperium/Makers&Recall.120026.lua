-- Bundled by luabundle {"rootModuleName":"Makers&Recall.120026.lua","version":"1.6.0"}
...
__bundle_register("Makers&Recall.120026.lua", function (require, _LOADED, __bundle_register, __bundle_modules)
    i18n = require("i18n")
    require("locales")
    ...
    pos_agent = {
        ...
        ["Yellow"] = { { 20.44, 2.75, 1.96 }, { 17.27, 2.75, 1.96 }, { 19.10, 2.75, 1.96 } }
    }
    ...
    function Makers()
        ...
        self.clearButtons()
        marker_round.setPositionSmooth({ -4.07, 1.8, -9.96 }, false, false)
        for i = 1, 3 do
            local agentCheck = 0
    ...
    function ConflictZone()
        marker_round.setPositionSmooth({ -4.05, 1.8, -10.62 }, false, false)
        ...
        for _, color in ipairs({ "Red", "Blue", "Yellow", "Green" }) do
            if mentat.hasTag(color) then mentat.removeTag(color) end
        end
        ...
        local Time = os.clock() +
            1 -- wait after collecting reveal zones into discards for letting time for cards to stack in the discard pile before drawing for holtzman effects
        while os.clock() < Time do coroutine.yield() end
        ...
        marker_round.setPositionSmooth({ -4.08, 1.8, -8.01 }, false, false)
        ...
        Wait.time(function () broadcastToAll(i18n("drawFive"), { 0.5, 0.5, 0.5 }) end,
            2)
        Wait.time(function ()
            marker_round.setPositionSmooth({ -4.06, 1.8, -8.65 }, false, false)
...
return __bundle_require("Makers&Recall.120026.lua")
