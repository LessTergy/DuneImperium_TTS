-- Bundled by luabundle {"rootModuleName":"Reclaimed Forces.3c772c.lua","version":"1.6.0"}
...
__bundle_register("Reclaimed Forces.3c772c.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
i18n = require("i18n")
require("locales")
...
function onLoad()
    ...
    self.createButton(data1)
    self.createButton(data2)
end
...
function tleilaxuScarab(_, color)
    ...
    if isEnoughSpecimen(_, color) then
        ...
        function(color)
            ...
            Wait.time(function()
                params = {color = color, silent = true}
                bene_tleilax_board.call("moveTleilaxuCall", params)
                broadcastToAll(leaderName .. i18n("reclaimedForcesScarab"),
                               color)
            end, 1)
        end)
    end
...
function spendSpecimen(color, price)
    Wait.time(function()
        params = {osef = "", color = color, silent = true}
        getObjectFromGUID("d5c2db").Call("RemoveSpecimenCall", params)
    end, 0.3, price)
end
...
return __bundle_require("Reclaimed Forces.3c772c.lua")
