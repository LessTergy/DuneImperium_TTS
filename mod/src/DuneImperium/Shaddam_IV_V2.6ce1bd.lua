-- Bundled by luabundle {"rootModuleName":"Shaddam IV V2.6ce1bd.lua","version":"1.6.0"}
...
__bundle_register("Shaddam IV V2.6ce1bd.lua", function (require, _LOADED, __bundle_register, __bundle_modules)
        ...
        if color == "Red" or color == "Blue" or color == "Green" or color == "Yellow" then
            ...
            for key, value in pairs(pion_reput) do
                Wait.time(function ()
                    ...
                    value[color].setPositionSmooth({ pos.x, pos.y, pos.z + 4.62 }, false, false)
                end, t)
                t = t + 1
            end
            Wait.time(function ()
                local x = 0
                if #getObjectFromGUID("4a3e76").call("getPlayersBasedOnHotseat") == 4 then
                    x = 1
                end
                for key, value in pairs(alliance_token) do
                    value.setPositionSmooth(vp_pos[color][5 + x], false, false)
                    x = x + 1
                end
            end, 5.3)
        else
...
return __bundle_require("Shaddam IV V2.6ce1bd.lua")
