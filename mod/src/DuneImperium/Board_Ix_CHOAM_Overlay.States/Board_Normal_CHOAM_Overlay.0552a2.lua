-- Bundled by luabundle {"rootModuleName":"Board Normal CHOAM Overlay.0552a2.lua","version":"1.6.0"}
...
__bundle_register("Board Normal CHOAM Overlay.0552a2.lua", function(require, _LOADED, __bundle_register, __bundle_modules)
i18n = require("i18n")
require("locales")
...
function sellMelange(color, spiceAmount, solariAmount)
    ...
    local val = ressources[color][1].call("collectVal")
    ...
    if boardCommonModule.CheckAgentAndPlayer(color, SellMelange_zone) then
        ...
        if val < spiceAmount then
            ...
            for i = 1, spiceAmount, 1 do
                ressources[color][1].call("decrementVal")
            end
...
function boardCommonModule.Swordmaster(color, zone)
    ...
    if boardCommonModule.CheckAgentAndPlayer(color, zone) then
        ...
        if solariObj.call("collectVal") < price then
            ...
            ressources[color][7].setPositionSmooth(ressources[color][8], false,
                                                   false)
            if leader.hasTag("Ilban") then
...
function boardCommonModule.drawIntrigue(color)
    if ressources[color][6].getObjects()[1].getName() == "Count Fenring" then
        x = 2
        Wait.time(function()
...
i18n.locales.en = {
    ...
    addNegociator = "+1 Negociator",
    spendNegociator = "-1 Negociator",
    negociatorAdded = "%s added a Negociator",
    negociatorRemoved = "%s removed a Negociator",
    negociatorLimitWarning = "You cannot place more than 4 Negociators with the automated script cause of the limited space on the board !!",
    addNegociatorManually = "Place your Negociator manually !!"
}
...
i18n.locales.fr = {
    ...
    addNegociator = "+1 Négociateur",
    spendNegociator = "-1 Négociateur",
    negociatorAdded = "%s a ajouté un Négociateur",
    negociatorRemoved = "%s a récupéré un Négociateur",
    negociatorLimitWarning = "Vous ne pouvez pas placer plus de 4 Négociateurs avec le script automatisé en raison de l'espace limité sur le plateau !!",
    addNegociatorManually = "Ajoutez manuellement votre Négociateur !!",
}
...
return __bundle_require("Board Normal CHOAM Overlay.0552a2.lua")
